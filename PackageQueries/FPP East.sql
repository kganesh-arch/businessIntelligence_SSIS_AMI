-- east fpp -- last updated 02/11/2021 to add GAAP
...
select
CERTIFICATE_NBR_BK, 
certificate_nbr_bk_amiwest, 
certificate_nbr_bk_ug, 
certificate_sk,
snapshot_month_dt_sk,loan_sk,
payment_lender_sk,
initial_premium_effective_dt_sk,
first_premium_rate_renewal_dt_sk,
second_premium_rate_renewal_dt_sk,
third_premium_rate_renewal_dt_sk,
fourth_premium_rate_renewal_dt_sk,	
property_state_sk,
originating_lender_branch_sk,
servicing_lender_branch_sk,
niw_book_year_dt_sk,
certificate_effecitive_termination_dt_sk,
certificate_status_change_dt_sk,
mi_credited_sales_agent_sk,
mi_credited_underwriting_office_sk,
mi_credited_sales_region_sk,
coverage_effective_dt_sk,
servicing_transfer_dt_sk,
current_cert_status_cd,
expected_renewal_premium_amt,
first_renewal_rate,first_renewal_term,
second_renewal_rate,second_renewal_term,
third_renewal_rate,
third_renewal_term,
fourth_renewal_rate,
fourth_renewal_term,
gross_earned_but_not_received_amt,
net_earned_but_not_received_amt,
change_in_net_ebnr_amt,
unearned_premium_amt,
change_in_unearned_premium_amt,
new_annual_premiums_applied_amt,
new_monthly_premiums_applied_amt,
new_singles_premiums_applied_amt,
renewal_annual_premiums_applied_amt,
renewal_monthly_premiums_applied_amt,
disclosure_remedies_accomodations_amt,
split_premium_surcharge_amt,
regular_premiums_applied_amt,
termination_refunds_amt,
refund_waiver_flag,
renewal_type,
pledged_amt,
disclosure_remedies_accomodations_etd_amt,
split_premium_surcharge_etd_amt,
financed_premium_flag,
average_insurance_in_force_amt,
Premium_Received_ETD_Amt,
Premium_Earned_ETD_Amt,
Premium_Written_ETD_Amt,
Premium_Refund_Credit_ETD_Amt,
...
--GAAP Measures below; BI-3742; Sep 2020 
,premium_earned_etd_amt_gaap
,premium_refund_credit_etd_amt_gaap 
,change_in_unearned_premium_amt_gaap
,unearned_premium_amt_gaap
-- Note that only the change_in_unearned_premium_amt_gaap part of this calculation is GAAP-eligible. Othere amounts in the calculations will stay the same as STAT
,coalesce(case when CURRENT_CERT_STATUS_CD<>'T' then (change_in_unearned_premium_amt_gaap+( regular_premiums_applied_amt + disclosure_remedies_accomodations_amt + split_premium_surcharge_amt ))*-1 else 0 end,0) as ammortization_earnings_amt_gaap
,coalesce(case when CURRENT_CERT_STATUS_CD='T' then (change_in_unearned_premium_amt_gaap+( regular_premiums_applied_amt + disclosure_remedies_accomodations_amt + split_premium_surcharge_amt ))*-1 else 0 end,0) as termination_earnings_amt_gaap
-- Note that only the gross_premiums_earned_gaap which uses change_in_unearned_premium_amt_gaap field is GAAP-eligible. Othere amounts in the calculations will stay the same as STAT
,coalesce(case when CURRENT_CERT_STATUS_CD <>'T' and (IN_FORCE_DAY>0  and IN_FORCE_MONTH >0 and IN_FORCE_YEAR > 0) and average_insurance_in_force_amt <> 0 then round((cast(gross_premiums_earned_gaap*12 as numeric(20,10))/average_insurance_in_force_amt)*100,6) else 0 end,0) as average_premium_yields_gaap
...
-- Used in calculation of Average_Premium_Yields_Gaap
,new_annual_premiums_applied_amt+new_monthly_premiums_applied_amt+new_singles_premiums_applied_amt+renewal_annual_premiums_applied_amt+renewal_monthly_premiums_applied_amt+change_in_net_ebnr_amt-change_in_unearned_premium_amt_gaap-termination_refunds_amt as gross_premiums_earned_gaap
			 
from
(
select
cast(CONCAT(pcm.CRT#CM,'*',pcm.BUIDCM) as varchar(20)) AS CERTIFICATE_NBR_BK,
NULL as certificate_nbr_bk_amiwest,
cast(pcm.CRT#CM as varchar(20)) as certificate_nbr_bk_ug,
...
pcm.STS@CM AS CURRENT_CERT_STATUS_CD, 
...
coalesce(uxc.POD$XC,0) as unearned_premium_amt, -- new code 
-- coalesce(coalesce(igx.unearned_premium_amt,0)-pul_FPP.unearned_premium_amt,0) as change_in_unearned_premium_amt,
coalesce(coalesce(uxc.POD$XC,0)-coalesce(pul_FPP.unearned_premium_amt,0),0) as change_in_unearned_premium_amt,
--coalesce(coalesce(igx.unearned_premium_amt,0)-coalesce(pul_FPP.unearned_premium_amt,0),0) as change_in_unearned_premium_amt,
...
coalesce(grs.net_earned_but_not_received_amt,0) as net_earned_but_not_received_amt,
coalesce(coalesce(grs.net_earned_but_not_received_amt,0)- coalesce(pne_FPP.net_earned_but_not_received_amt,0),0) as change_in_net_ebnr_amt,
...
coalesce(lndr.lender_branch_sk,(select lender_branch_sk from edw_dm.dbo.DIM_LENDER where lender_branch_bk = '-1')) as payment_lender_sk,
...
coalesce(olndr.lender_branch_sk,(select lender_branch_sk from edw_dm.dbo.DIM_LENDER where lender_branch_bk = '-1')) as originating_lender_branch_sk,
coalesce(slndr.lender_branch_sk,(select lender_branch_sk from edw_dm.dbo.DIM_LENDER where lender_branch_bk = '-1')) as servicing_lender_branch_sk,
...
--GAAP Measures below;  BI-3742; Sep 2020
,coalesce(uxc_gaap.UEL$XC,0) as premium_earned_etd_amt_gaap
,coalesce(uxc_gaap.URL$XC,0) as premium_refund_credit_etd_amt_gaap 
,coalesce(coalesce(uxc_gaap.POD$XC,0)-coalesce(pul_FPP.unearned_premium_amt_gaap,0),0) as change_in_unearned_premium_amt_gaap
,coalesce(uxc_gaap.POD$XC,0) as unearned_premium_amt_gaap

from	
dbo.STG_ISPFPCM_E pcm
left outer join
(
	select 
											       ...
											       ,PSCYTH*10000+PSCMTH*100+PSCDTH DESC
											        ,RLCYTH*10000+RLCMTH*100+RLCDTH DESC
															  ,case when ath.LVIDTH <> PCM.lvidcm or ath.lvbrth <> pcm.lvbrcm or ath.lvckth <> pcm.lvckcm then 2 else 1 end asc
																												...
																												from   dbo.STG_ISPFATH_E ATH
																												
		   join dbo.stg_ispfpcm_E pcm
...
on pcm.CERT#CM = plm.certificate_nbr_bk
...
on pcm.CERT#CM = rss.CERTIFICATE_NBR_BK
...
on pcm.CERT#CM = stm.CERTIFICATE_NBR_BK and stm.record_last_changed_date = stm.max_record_last_changed_date

left outer join
(
	SELECT distinct
		CERT#CM AS CERTIFICATE_BK,
		CASE WHEN
	...
	FROM 
		dbo.STG_ISPFPCM_E, dbo.stg_ispfgur_E, stg_ispflbr_E
	where
		srnbcm = urnbur and
		loidcm = lidnbr and
		lobrcm = lbrnbr
) cw
on pcm.CERT#CM = cw.CERTIFICATE_BK
...
(
	...
	FROM
		dbo.STG_ISPFUIGX_SYSE sy
		join edw_staging.dbo.stg_ispfpcm_E pcm2
		on pcm2.CERT#CM=CTIDIG * 10 + CTCKIG
		left join edw_dm.dbo.DIM_MI_CERTIFICATE mi
		--on CTIDIG * 10 + CTCKIG = mi.CERTIFICATE_NBR_BK 
		on cast(pcm2.CRT#CM as varchar) = mi.CERTIFICATE_NBR_BK_ug
		and mi.curr_ind = 'Y'
	GROUP BY
		CTIDIG * 10 + CTCKIG
) igx
on pcm.CERT#CM = igx.CERTIFICATE_NBR_BK
...
(
	...
	and pul_dmic.certificate_nbr_bk_ug is not null
...
on pcm.CRT#CM = pul_DMIC.CERTIFICATE_NBR_BK_ug
...
(
	...
	from
		dbo.STG_ISPFAPW_E
		join edw_staging.dbo.stg_ispfpcm_E pcm1
		on pcm1.CERT#CM=CTIDPW * 10 + CTCKPW
		left join edw_dm.dbo.DIM_MI_CERTIFICATE mi
		--on CTIDPW * 10 + CTCKPW = mi.CERTIFICATE_NBR_BK 
		on pcm1.CRT#CM  = mi.CERTIFICATE_NBR_BK_ug
		and mi.curr_ind = 'Y'
	group by
		CTIDPW * 10 + CTCKPW
) apw
on pcm.CERT#CM = apw.certificate_nbr_bk

left outer join
(
	dbo.STG_ISPFPCM_E sps_PCM inner join dbo.stg_ISPFRCT_E sps_RCT
	on sps_PCM.CERT#CM = CTIDCT*10+CTCKCT
	and PCV@CM IN ('PB','PD','PE','PF','PG','PI','PJ','PM','PO','PP','PQ','PU','RK','RM', 'S0', 'SM')
	and PPRDCT IN ('SP1','SP2')
)
on pcm.CERT#CM = sps_pcm.CERT#CM

left outer join dbo.stg_ISPFPAM_E pal
on pcm.CERT#CM = pal.CTIDAM*10 + pal.CTCKAM

left outer join
(
	dbo.STG_ISPFPCM_E spse_PCM inner join dbo.STG_ISPFRCT_E spse_RCT
	on spse_pcm.CERT#CM = CTIDCT*10+CTCKCT
	and PCV@CM IN ('PB','PD','PE','PF','PG','PI','PJ','PM','PO','PP','PQ','PU','RK','RM', 'S0', 'SM')
	and PPRDCT IN ('SP1','SP2')
) 
on pcm.CERT#CM = spse_pcm.CERT#CM

left outer join
(
	 Select 
		Cast(REPLACE(certificate_sk,'-','')as Numeric) as Certificate_Nbr_Bk, 
		sum(gross_earned_but_not_received_amt) as gross_earned_but_not_received_amt,
		sum(net_earned_but_not_received_amt) as net_earned_but_not_received_amt
	from
		edw_dm.dbo.fct_ebnr_ageing_E
		where snapshot_month_dt_sk=(@SnapshotYear * 10000) + (@SnapshotMonth * 100) + @SnapshotDay
		group by certificate_sk
	) grs
...
(
	Select 
		...
		sum(month_1) + sum(month_2) + sum(month_3) + sum(month_4) +
		sum(month_5) + sum(month_6) + sum(month_7) + sum(month_8) + sum(month_9) + sum(month_10) +
		sum(month_11) + sum(month_12) + sum(month_13_16) as gross_earned_but_not_received_amt
	from
...
(
	...
	and pne_DMIC.certificate_nbr_bk_ug is not null -- added filter on 07/26
...
on pcm.CRT#CM = pne_DMIC.CERTIFICATE_NBR_BK_ug
...
(
	...
	WHERE	
		...
		AND CURRENT_FLAG = 'Y'
		AND	(PTR@OI = 'ADJ' AND (RSN@OI = 'TR' OR RSN@OI IN(SELECT SUBSTRING(TVALTB,1,2) FROM DBO.STG_ISPFXTB_E WHERE TNAMTB = 'TERMTYPE') and isnull(PSU@OI,'ZZZ')<> 'AI'))
	GROUP BY 
		CTIDOI*10+CTCKOI,PTYOI,PTMOI
) trora
on pcm.CERT#CM = trora.CERTIFICATE_NBR
...
(
	...
	WHERE	
		...
		AND  EXISTS 
		(
			SELECT * FROM DBO.STG_ISPFATH_E B
...
on pcm.CERT#CM = trpcr.CERTIFICATE_NBR
...
(
	...
	WHERE	
		...
		AND  EXISTS 
		(
			SELECT * FROM DBO.STG_ISPFAOI_E B
...
on pcm.CERT#CM = trocr.CERTIFICATE_NBR
...
(
	...
	WHERE 
		...
		AND PURMTH=PTMTH
		AND	PTR@TH='ADJ'
		and (psu@th = 'AV' AND (RSN@TH='TR' OR RSN@TH IN(SELECT SUBSTRING(TVALTB,1,2) FROM DBO.STG_ISPFXTB_E  WHERE TNAMTB= 'TERMTYPE'))
		OR psu@th = 'AD' AND (RSN@TH='TR' OR RSN@TH IN(SELECT SUBSTRING(TVALTB,1,2) FROM DBO.STG_ISPFXTB_E  WHERE TNAMTB= 'TERMTYPE'))
		and  Exists
		(
			Select * from DBO.STG_ISPFATH_E B
...
on pcm.CERT#CM = trra.CERTIFICATE_NBR
...
(
	...
	and pm_mi.certificate_nbr_bk_ug is not null
...
on pcm.CRT#CM  = pm_MI.certificate_nbr_bk_ug

left outer join dbo.stg_ispfuxc_E uxc
on pcm.CERT#CM = uxc.CTIDXC * 10 + uxc.CTCKXC

left outer join edw_dm.dbo.DIM_MI_CERTIFICATE mi
--on pcm.CRT#CM = mi.certificate_nbr_bk 
on pcm.CRT#CM  = mi.CERTIFICATE_NBR_BK_ug
and mi.curr_ind = 'Y'

left outer join edw_dm.dbo.DIM_LOAN ln
--on pcm.CRT#CM = ln.certificate_nbr_bk 
on pcm.CRT#CM = ln.CERTIFICATE_NBR_BK_ug
and ln.curr_ind = 'Y'
...
left outer join edW_staging.dbo.stg_ispfuxc_E_gaap uxc_gaap 
on pcm.CERT#CM = uxc_gaap.CTIDXC * 10 + uxc_gaap.CTCKXC
) x
) y